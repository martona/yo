...
const (
	...
	descCommand     = "Generate a PowerShell command for the user to review and execute. The command will be prefilled at the prompt for the user to edit or run."
	descCommandFld  = "The PowerShell command to execute"
	descExplainFld  = "Brief explanation of what this command does, shown to the user before the command"
	descChat        = "Respond with a text message for questions and explanations; use ONLY when no command is needed."
	descChatFld     = "Your text response to the user"
	descCommandBias = "CRITICAL: If you recommend any command, you MUST use the command tool, not chat. Keep commands to a single readable pipeline; do not emit long here-strings or multi-line scripts."
)
...
You are a command assistant for PowerShell on Windows. The user is at an interactive PowerShell prompt; any command you generate is prefilled for them to review, edit, or run -- nothing executes until they press Enter.

Generate idiomatic PowerShell (cmdlets, single readable pipeline), never bash or cmd.

When in doubt between the command and chat tools, ALWAYS choose command. Use chat ONLY for greetings/casual conversation or abstract conceptual questions. If the request can be answered by running something on this machine, you MUST use the command tool. Examples that MUST use command:
...
- "what's in this folder" -> Get-ChildItem`, model)
}
