...
**v0.2.x ships for Windows (PowerShell 5.1 and 7+), macOS (zsh + Homebrew bash 4.2+), and Linux (bash 4.2+ + zsh) -- with signed/notarized release builds, native Linux packages (.deb/.rpm/Arch plus a static binary and a `curl | bash` installer), and CI on all three OSes.** `yo <text>` calls the LLM and prefills a shell command on the next prompt (or prints a chat answer); live-verified across Anthropic, OpenAI, Grok (xAI), and Gemini (Google), including multi-step sequences.

- **Layout:** `cmd/yo/` (entry) + `internal/config` + `internal/llm` (a `Provider` interface with `anthropic.go`, `openai.go`, and `chatcompletions.go` -- the last a shared OpenAI-compatible Chat Completions client used by both Grok and Gemini). Build: `go build ./cmd/yo`. Tests live beside the code; `go test ./...` is green.
- **Providers (four):** selected via `provider` in `~/.yoconf` or inferred from whichever key env var is set -- `anthropic` (Messages API, `ANTHROPIC_API_KEY`, default `claude-opus-4-8`), `openai` (Responses API, `OPENAI_API_KEY`, `gpt-5.5`), `grok` (xAI, OpenAI-compatible **Chat Completions**, `XAI_API_KEY`, `grok-4.3`), and `gemini` (Google's OpenAI-compat endpoint, **Chat Completions**, `GEMINI_API_KEY`, `gemini-3.5-flash`). Grok and Gemini share `chatcompletions.go`; all four force tool use and route by tool name. A `base_url` override (proxy / compatible endpoint) is supported per provider. See [`yoconf.example`](../yoconf.example).
- **Shell integration:** `shell/yo.ps1`, `shell/yo.zsh`, and `shell/yo.bash` — per-shell adapters over the shared Go core. PowerShell uses a one-shot OnIdle next-prompt prefill plus a wrapped `prompt`; zsh uses ZLE plus `preexec`/`precmd`; bash uses a Readline `bind -x` Enter hook and `PROMPT_COMMAND` cleanup. `.ps1` files are kept pure ASCII (PowerShell 5.1 chokes on smart punctuation), and the snippet forces `[Console]::OutputEncoding = UTF-8` so non-ASCII replies render. Key/config files are decoded tolerantly (UTF-8 / UTF-8-BOM / UTF-16).
- **Raw-line capture:** a PSReadLine Enter hook single-quotes a `yo <query>` line before PowerShell parses it, so a question can contain `( ) < > & ; | $` without manual quoting; debug-flag calls (`yo --dry-run …`), already-quoted lines, and all non-`yo` input pass through untouched. Requires PSReadLine (pwsh 7+) with a guarded fallback. See [Raw-line capture (PowerShell)](#raw-line-capture-powershell).
...
- **Scrollback:** inside **zellij** or **tmux**, recent screen output is captured and folded into the query as context ("why did that fail?") — `zellij action dump-screen --full --path <file>` or `tmux capture-pane -p -S -<lines>`, ANSI-stripped, last 200 lines, then secrets scrubbed before send (`internal/scrollback` + `internal/redact`). Outside multiplexers: a no-op on Unix, but on Windows it falls back to the console buffer (viewport under Windows Terminal, full buffer under conhost).
- **Output-fed continuation:** **done** — continuation steps now fold in scrollback the same way the initial query does, so the model reacts to each step's real output, not just its exit code. **Secret redaction:** **done** too — outbound scrollback is scrubbed by gitleaks' engine (imported in-process, embedded ruleset) before send, on both paths (`internal/redact`). **Cross-call session memory:** **done** too — a per-session history of exchanges is folded into later queries (`internal/session`).
...
The app's real work is: HTTPS calls with auth headers, JSON parsing, session memory across invocations, shelling out to terminal multiplexers and post-processing their output, and entropy-based secret filtering. In shell languages every one of these is a fight — JSON means depending on `jq`, HTTPS means string-building `curl` headers, memory means inventing a tempfile serialization scheme.

The decisive factor is **code sharing**, not convenience. A `.sh`/`.ps1` split is not two thin wrappers around shared logic — it is **two complete reimplementations** of the entire brain (API client, JSON handling, prompt assembly, response parsing, multiplexer capture, secret filtering), in two languages that share nothing and fail differently. The script approach has the same surface area as native, just duplicated across two runtimes.

A native binary inverts this: the entire brain is written **once** and cross-compiled. The only per-platform, per-shell code left is the prefill integration snippet — small precisely because it's the only thing that legitimately must be shell-specific.
...
### Token accounting (as built)

`yo --tokens` prints input/output token totals for the current shell session and all-time (`yo --tokens-reset` clears the all-time counter). **Tokens only, no dollar cost** — every provider returns `usage` in the response body (Anthropic and OpenAI-Responses: `input_tokens`/`output_tokens`; Grok/Gemini Chat Completions: `prompt_tokens`/`completion_tokens`), so the counts are exact and free, no extra call. Cost was deliberately left out: the APIs don't return it, a price table goes stale, and a wrong number is worse than none.

**Flow.** Each provider parser folds the reported usage onto the `Result` (`InputTokens`/`OutputTokens`, `json:"-"` — *not* part of the binary↔snippet contract: neither the JSON nor the sh encoder emits them); `main`/`runContinue` add it to the tally after every call — the initial query and each `--continue` step, so a multi-step task sums naturally. `--tokens` / `--tokens-reset` are pure local reads handled like `--config` (no network, no key), and the snippet passes them straight through (a `-*` arg bypasses parsing).

**Storage (`internal/usage`).** A single JSON file — global counters plus a per-session map keyed by `$env:YO_SESSION` — under the OS **user-config dir** (`os.UserConfigDir()/yo/usage.json`; `$env:YO_USAGE_DIR` overrides). Persistent *on purpose*, unlike the temp-dir session store: an all-time total must survive reboots and reset only on `--tokens-reset` (in temp it would silently zero on a temp sweep). It holds only integer counts, a session id, and timestamps — no query text, no output — so it is not redaction-relevant. Per-session entries age out after 7 days idle. This is yo's first *persistent* (non-temp) footprint; accumulation is always-on (it's the only way to have something to report), with a `tokens false` opt-out left as a possible future directive. **`yo --uninstall` offers to delete it** (and the temp session-memory cache) in a shared, cross-shell Go step that runs after the per-shell profile removal — and deliberately leaves `~/.yoconf` (your provider + key) in place, saying so, since that file is yours, not yo-generated state.
...
- **Providers:** four — **Anthropic** (Messages API) and **OpenAI** (Responses API) first, then **Grok** (xAI) and **Gemini** (Google) via a shared OpenAI-compatible Chat Completions client (`chatcompletions.go`). API keys via the standard `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `XAI_API_KEY` / `GEMINI_API_KEY` env vars (with an optional `key` override in `~/.yoconf`). Key files (yoshell's `~/.anthropickey`) are deliberately not supported — env vars are the expected path.
...
**Bash as built.** Bash mirrors the zsh model (bash has no `print -z`). An accept-line hook (`_yo_rewrite_buffer`, an Enter macro: a `bind -x` widget then `accept-line`) rewrites a raw `yo <query>` line to a safely single-quoted `yo '<query>'` and accepts it -- so the typed line is preserved on screen and metacharacters never reach the parser. The `yo` function then runs as an ordinary command and prints its explanation/answer cleanly below. For a command result, the next (editable) command is placed on the **next** prompt via the terminal's DSR reply (`ESC[5n` -> bound `ESC[0n`), the bash analogue of `print -z`; the kernel's echo of that reply is suppressed with `stty -echo` and restored in `_yo_mark_prompt` (last in `PROMPT_COMMAND`, so both initial and continuation-step prefills render). Multi-step continuation is driven by `PROMPT_COMMAND` (the precmd, `_yo_precmd`) plus a **`DEBUG` trap (preexec, `_yo_preexec`)** that, gated by an at-prompt flag, detects that the user actually ran a command and captures it (`YO_RAN`). This detection is tied to real command execution -- reliable across bash versions and immune to history settings / `prefill_space`; an earlier history-based and an accept-widget-side-effect approach both failed (the latter because a `bind -x` widget's *global-variable* writes don't persist when invoked via a key macro, though its `READLINE_LINE` writes do). Bare-Enter and Ctrl-C cancel (no command runs -> the trap never flags one). The minimum is bash 4.2+; macOS `/bin/bash` 3.2 is deliberately unsupported.

The same DSR/`print -z`-style detail and the three-tries-to-a-DEBUG-trap history are in [LINUX-PORT.md](LINUX-PORT.md).
...
**The `thinking...` indicator on this path.** The binary prints the transient `thinking...` to stderr (cleared with `\r`), which paints fine for the initial `yo` call. But the `--continue` call runs *from the prompt function*, where PSReadLine owns the prompt region and a **child** process's transient, self-clearing stderr write doesn't end up visible — whereas a write from the **shell** process itself does. So on the continuation the snippet renders the indicator (`[Console]::Error.Write`) and passes `--no-thinking` to silence the binary's copy: one writer, no double. zsh is unaffected (its `precmd` has no such repaint), so the binary keeps printing it there. The whole call sits inside the prompt's `try/catch` (plus a `finally` that always clears), so a console-write failure can't wedge the prompt. *(Mechanism note: it is not stream-swallowing — native stderr does reach the console even from a collected scriptblock; the loss is specific to the transient in-place write under interactive prompt rendering.)*
...
**Simplicity nudge (leaned the knob).** Observed behavior: for a question like "is my USB stick mounted" the model emitted a ~200-char one-liner (a `Win32_DiskDrive -> DiskPartition -> LogicalDisk` CIM walk, `pending:false`) — correct, but neither simple nor reusable. Two causes: (1) PowerShell's object pipeline makes the *precise* answer cheap to compose in one shot (bash's verbose tools — `lsblk`, `mount` — instead invite "run it, read it", a natural two-step); (2) **our own prompt drift** — the system prompts said *"prefer a single readable pipeline"* in three places, a PowerShell-adaptation slip from yoshell's *"short, focused commands"* that actively rewarded the megapipe. Fix: replaced all three with *"prefer the simplest, most reusable command... not an intricate one-liner built to compute the exact answer"*, and added a `command`-tool rule — when precision would otherwise need a long/multi-stage pipeline, run a simple well-known diagnostic with `pending=true` and read its output to answer. Goal: a command the user can learn and reuse, then an evaluation, over a clever one-liner. Still stochastic; the `debug` trace (above) is how we watch the `pending` flag to see if it's landing.
...
1. ~~**Scrollback (Windows-native).**~~ **Done** — zellij and tmux folded into the query as a context block (`internal/scrollback` + `llm.WithTerminalContext`), ANSI-stripped, last 200 lines; Windows console fallback built too. Now folded into continuation steps as well (the `--continue` path), not just the initial query, and the captured text is redacted before send.
...
4. ~~**More providers.**~~ **Done — four total.** OpenAI (Responses, worked-example tuned prompt), then **Grok** (xAI) and **Gemini** (Google), the latter two on a shared OpenAI-compatible Chat Completions client (`chatcompletions.go`) — so adding each was just a constructor + a `config` defaults entry + a `--setup` menu line. All force tool use and route by tool name; Grok and Gemini both live-verified. The `docs` tool was **dropped** (by the time a user can ask "how do I configure yo?", they're past setup; `--help`/`--config`/README cover it).
...
1. **CLI surface + installer.** **Done:** `--help` (curated), `--version` (ldflags `-X main.version=<tag>` in CI, `debug.ReadBuildInfo` fallback), `--config`, `--init powershell`, `--init zsh`, and `--init bash` — which emit embedded shell snippets via `//go:embed` (`shell/embed.go`), version-locked to the binary. Manual install is the profile one-liner for the target shell (`yo --init powershell | Out-String | iex` in `$PROFILE`, `eval "$(yo --init zsh)"` in `~/.zshrc`, or `eval "$(yo --init bash)"` in `~/.bashrc`). Human-facing flags print plain text, never the stdout JSON contract. **Also done:** `yo --setup` / `--uninstall` (flags, not bare subcommands — those would collide with NL queries). Setup is now orchestrated by Go (`cmd/yo/setup.go`): it offers every supported profile it can sensibly edit rather than choosing only the current shell, owns the shared provider/key prompt, writes `provider` and `key` to `~/.yoconf` (`0600`) when approved, and ends in `yo --check`. PowerShell setup delegates only PowerShell-native work to the embedded `shell/setup.ps1`: adding the managed init block to `$PROFILE`, checking/upgrading PSReadLine `-Scope CurrentUser`, and adding the binary's dir to user PATH if missing. The helper runs under the *invoking* PowerShell host (parent-process detection, `cmd/yo/parent_windows.go`) so it wires the right profile on **5.1 or 7+**. zsh and bash setup share a Go-native POSIX profile helper (`cmd/yo/setup_posix.go`): when `yo` resolves on PATH, zsh detects `${ZDOTDIR:-$HOME}/.zshrc`, bash uses `$HOME/.bashrc`, both write the same managed init-block convention, and uninstall removes only that managed block or exact legacy init line; when `yo` is not on PATH, setup offers to copy the current binary to `~/.local/bin/yo`, adds a managed PATH block to each accepted POSIX profile if needed, then writes the same normal init block. **Confirm-before-change: done.** Each modifying step previews what it will do and asks first; a "no" skips *only that step* and setup proceeds — declining one never aborts the run. The provider prompt is a `1/2` menu (1 = Anthropic, 2 = OpenAI; typed names still accepted), not free-typed text. `--uninstall` likewise confirms before editing the shell profile. Read-only steps (PATH/key already resolve, PSReadLine current where relevant, already wired, final `--check`) ask nothing. **Exit codes documented** in `--help`: `0` success, `1` runtime error (config / key / network / API), `2` usage error (no query, or an unknown `--init` shell) — `2` is the Unix usage-error convention, broader than the original "no query" note. **Item 1 is now complete.**
2. **README + license/docs.** **README: done** -- `README.md` at the repo root makes the GPLv3-derivative statement *user-visible* (deferring to `NOTICE` for specifics), with status, install (build / `go install` + `yo --setup`, or manual), usage (incl. the investigate-then-evaluate and "why did that fail" examples), a config table (incl. `debug`), the safety stance, an explicit no-telemetry note, how-it-works, a flags table, and license/provenance. **Still open (all distribution-time):** transitive-license **aggregation** for the ~62 bundled modules into a `THIRD-PARTY-LICENSES` file (`go-licenses save`/`report`/`check`) -- a GPLv3 obligation once *binaries* ship, not while it's source-only; an optional **SBOM** (`cyclonedx-gomod app`, CycloneDX/SPDX) for vuln-scanning, cheap to add to the same CI step; verify SPDX header coverage; settle the prompt.go gofmt convention (it was committed gofmt-clean). **Dropped:** the `docs` config blurb ("how do I configure yo" from embedded help) -- by the time a user can ask that, they are past setup and `--help`/`--config`/the README already cover it (author's call, 2026-06-21).
3. **Polish.** Friendly-error audit (missing key / network / API 4xx-5xx / bad config, no prefill); confirm Ctrl-C cancels an in-flight request; verify the default model ids are current; ~~`yoconf.example` documents every directive incl `memory`~~ **done** — now also documents the new `debug` toggle; `-trimpath -ldflags "-s -w"` to shrink the ~15 MB binary.

   **Debug tracing (as built).** A `debug` directive in `~/.yoconf` (or `$env:YO_DEBUG`, which overrides it per-session) turns on a one-line stderr trace of the *scaffolding* around each LLM call: `yo[debug] -> <provider>/<model>  q="…"  [scrollback +Nch, memory +Nch]` before the call and `yo[debug] <- command pending=<bool>  "…"` (or `<- chat`) after. Deliberately lightweight — it shows the **sizes** of attached context, never the scrollback or command output itself (`--dry-run` remains the full-request view). Off by default; written to stderr so the stdout JSON contract is untouched (`dbg`/`dbgResult` in `cmd/yo/main.go`, `Config.Debug` + `truthy` in `internal/config`). Its first job: making it visible whether a one-shot answer was the model's call (`pending=false`) or a continuation that misfired.
4. **CI.** GH Actions now covers Windows and macOS. Windows CI builds/tests/vets, cross-compiles amd64 + arm64, parses the PowerShell snippets, and enforces pure ASCII for `.ps1` files. macOS CI builds/tests/vets, parses `shell/yo.zsh` and `yo --init zsh`, and builds `darwin/amd64` plus `darwin/arm64`. Linux remains deferred.
5. **Signing.** **Done for release builds:** Windows binaries are Authenticode-signed via Azure Trusted Signing. macOS binaries are Developer ID-signed with hardened runtime and notarized by Apple, using the same Apple secrets/vars shape as `martona/clipp`.
6. **Packaging.** **Manifests built (`packaging/`), submission pending the author's accounts.** scoop (`packaging/scoop/yo.json` -- per-arch zips, real hashes, `checkver`+`autoupdate` self-maintaining from `SHA256SUMS.txt`, `bin: yo.exe`; ship via own bucket or PR to `ScoopInstaller/Extras`) + winget (`packaging/winget/*.yaml` for `martona.yo` -- zip/nested-portable `yo.exe`->alias `yo`; seed once then `.github/workflows/winget.yml` auto-bumps via `winget-releaser` given a `WINGET_TOKEN`). Both only put `yo.exe` on PATH, so the manifests direct users to `yo --setup`; full steps in `packaging/README.md`. `go install github.com/martona/yo/cmd/yo@latest` works (repo is public). **Skipped MSIX** (sandbox fights a `$PROFILE`-editing, PATH-needing, shell-hooking CLI; revisit only for a Store presence). chocolatey optional.
...
### CI + release pipeline (built 2026-06-21)

GitHub Actions, adapted from the author's `../WM_NIGHT` Windows bones (same Azure Trusted Signing setup), simplified for a single Go binary. Four workflows under `.github/workflows/`:

- **`windows-ci.yml`** (push/PR to master): `gofmt -l` gate, `go vet`, `go test`, cross-compile both shipped Windows arches (amd64 + arm64), and the two snippet guards CI can give us -- a **PowerShell parse smoke** and a **pure-ASCII check** of `shell/yo.ps1` + `shell/setup.ps1`.
- **`macos-ci.yml`** (push/PR to master): `gofmt -l`, `go vet`, `go test`, `zsh -n shell/yo.zsh`, `yo --init zsh | zsh -n`, and `darwin/amd64` + `darwin/arm64` builds.
- **`_release.yml`** (reusable): shared **license aggregation** (`go-licenses save`) + **SBOM** (`cyclonedx-gomod`) -> Windows amd64+arm64 build with `-trimpath -ldflags "-s -w -X main.version=<tag>"` -> **Azure Trusted Signing** (`azure/login` OIDC + `azure/trusted-signing-action`, gated on the `release-signing` environment) -> macOS amd64+arm64 build -> **Developer ID signing + Apple notarization** (`apple-actions/import-codesign-certs`, `xcrun notarytool --wait`) -> per-platform/per-arch **zip** of the loose binary + `LICENSE`/`NOTICE`/`README`/`third-party-licenses/` -> `SHA256SUMS.txt` -> `attest-build-provenance` -> `softprops/action-gh-release` (drafts by default). Version-less asset names (`yo-windows-<arch>.zip`, `yo-macos-<arch>.zip`) provide stable `releases/latest/download` URLs.
- **`release-tag.yml`** / **`release-manual.yml`**: triggers that resolve the version (from the pushed `v*` tag, or a `workflow_dispatch` input) and call `_release.yml`.

Reuses the author's existing signing config: Azure secrets/vars for Windows plus Apple Developer ID/App Store Connect secrets/vars for macOS, all gated by the `release-signing` environment. See [RELEASING.md](RELEASING.md). **Deliberately dropped:** MSIX (sandbox fights a PATH/`$PROFILE`-editing CLI). **Still deferred:** Linux build legs and actual package-manager submission.
