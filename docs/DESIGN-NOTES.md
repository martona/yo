...
1. **CLI surface + installer.** **Done:** `--help` (curated), `--version` (ldflags `-X main.version=<tag>` in CI, `debug.ReadBuildInfo` fallback), `--config`, `--init powershell`, `--init zsh`, and `--init bash` — which emit embedded shell snippets via `//go:embed` (`shell/embed.go`), version-locked to the binary. Manual install is the profile one-liner for the target shell (`yo --init powershell | Out-String | iex` in `$PROFILE`, `eval "$(yo --init zsh)"` in `~/.zshrc`, or `eval "$(yo --init bash)"` in `~/.bashrc`). Human-facing flags print plain text, never the stdout JSON contract. **Also done:** `yo --setup` / `--uninstall` (flags, not bare subcommands — those would collide with NL queries). Setup is now orchestrated by Go (`cmd/yo/setup.go`): it selects the target shell, owns the shared provider/key prompt, writes `provider` and `key` to `~/.yoconf` (`0600`) when approved, and ends in `yo --check`. PowerShell setup delegates only PowerShell-native work to the embedded `shell/setup.ps1`: adding the init line to `$PROFILE`, checking/upgrading PSReadLine `-Scope CurrentUser`, and adding the binary's dir to user PATH if missing. The helper runs under the *invoking* PowerShell host (parent-process detection, `cmd/yo/parent_windows.go`) so it wires the right profile on **5.1 or 7+**. zsh and bash setup are Go-native because the required operations are plain file edits: zsh detects `${ZDOTDIR:-$HOME}/.zshrc`, bash uses `$HOME/.bashrc`, both write a managed init block with an optional pinned `YO_BIN` fallback, and uninstall removes only that managed block or exact init line. **Confirm-before-change: done.** Each modifying step previews what it will do and asks first; a "no" skips *only that step* and setup proceeds — declining one never aborts the run. The provider prompt is a `1/2` menu (1 = Anthropic, 2 = OpenAI; typed names still accepted), not free-typed text. `--uninstall` likewise confirms before editing the shell profile. Read-only steps (PATH/key already resolve, PSReadLine current where relevant, already wired, final `--check`) ask nothing. **Exit codes documented** in `--help`: `0` success, `1` runtime error (config / key / network / API), `2` usage error (no query, or an unknown `--init` shell) — `2` is the Unix usage-error convention, broader than the original "no query" note. **Item 1 is now complete.**
