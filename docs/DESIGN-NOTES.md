...
1. **CLI surface + installer.** `--help` (curated, not flag's default), `--version` (`-ldflags "-X main.version=<tag>"` in CI, `debug.ReadBuildInfo` fallback), `--config` (show resolved settings + paths), documented exit codes (2 = no query, 1 = error). A `yo setup` subcommand (single-artifact install) + uninstall: idempotent `$PROFILE` append (at the END, after any custom prompt), a PSReadLine 2.x / pwsh 7+ check (the 5.1 prefill garble), binary placement + PATH, a key/provider prompt -> `~/.yoconf`, ending in `yo --check`. Human-facing flags print plain text, never the stdout JSON contract.
