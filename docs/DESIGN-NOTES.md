...
- **`windows-ci.yml`** (push/PR to master): `gofmt -l` gate, `go vet`, `go test`, cross-compile both shipped arches (amd64 + arm64; Go cross-compiles, so no arm64 runner), and the two snippet guards CI can give us -- a **PowerShell parse smoke** and a **pure-ASCII check** of `shell/yo.ps1` + `shell/setup.ps1`.
- **`_release.yml`** (reusable): cross-compile amd64+arm64 with `-trimpath -ldflags "-s -w -X main.version=<tag>"` (verified: bakes the tag into `yo --version`, ~11 MB exe) -> **Azure Trusted Signing** (`azure/login` OIDC + `azure/trusted-signing-action`, gated on the `release-signing` environment) -> **license aggregation** (`go-licenses save`, verified: 313 files, exit 0) + **SBOM** (`cyclonedx-gomod app -licenses -assert-licenses`, verified: 62 components, all permissive/GPLv3-compatible BSD/MIT/Apache) -> per-arch **zip** of the loose exe + `LICENSE`/`NOTICE`/`README`/`third-party-licenses/` -> `SHA256SUMS.txt` -> `attest-build-provenance` -> `softprops/action-gh-release` (drafts by default). Version-less asset names (`yo-windows-<arch>.zip`) for a stable `releases/latest/download` URL.
...
Reuses the author's existing signing config verbatim -- secrets `AZURE_CLIENT_ID`/`AZURE_TENANT_ID`/`AZURE_SUBSCRIPTION_ID`, vars `ARTIFACT_SIGNING_ENDPOINT`/`ARTIFACT_SIGNING_ACCOUNT`/`ARTIFACT_SIGNING_CERTIFICATE_PROFILE`, and a `release-signing` environment whose Azure federated credential is scoped to `repo:martona/yo:environment:release-signing`. **Pending (author, Azure/GitHub side):** create that environment + federated credential + secrets/vars for `martona/yo`, then push a `v*` tag for the first (draft) release. **Deliberately dropped:** MSIX (sandbox fights a PATH/`$PROFILE`-editing CLI). **Still deferred:** winget/scoop manifests (a follow-on step once releases exist); macOS/Linux build legs. Once the first release publishes, add a "Download" section to the README (it currently documents build-from-source only -- correct until binaries exist).
