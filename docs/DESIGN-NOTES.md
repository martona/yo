...
**v0.2.x ships for Windows (PowerShell 5.1 and 7+) and macOS (zsh), with signed/notarized release builds; main also has bash 4.2+ integration.** `yo <text>` calls the LLM and prefills a shell command on the next prompt (or prints a chat answer); live-verified across Anthropic, OpenAI, Grok (xAI), and Gemini (Google), including multi-step sequences.
...
**Bash as built.** Bash has no zsh-style `print -z`, so the adapter keeps the initial and pending-step flow inside Readline. Enter is rebound to a macro that runs `_yo_readline_enter` and then an internal finish key. For ordinary commands that finish key is `accept-line`; for `yo <query>` it is `redraw-current-line` after the hook has called the binary and replaced `READLINE_LINE` with the offered command. Pending steps are also run from the hook, in the current shell via `eval`, so `cd`/`export` side effects survive; the hook records history, captures the edited command as `YO_RAN`, calls `yo --continue`, and replaces `READLINE_LINE` with the next offering. Ctrl-C at an armed prompt is cleaned by a small `PROMPT_COMMAND` hook. The minimum is bash 4.2+; macOS `/bin/bash` 3.2 is deliberately unsupported.
